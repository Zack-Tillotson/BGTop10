...
export function calculateTagGameList(lists: Ranking[]) {
  ...
  lists.forEach((list) => list.gameLink.forEach(({games}) => {
    games.forEach((bggId, index, {length: linkCount}) => {
      ...
      const placementValue = index/linkCount*10 // range 10-1, ie 10 for the first 10% of items in a list, 1 for the last 10%
      gameMap[bggId] += listMultiplier * placementValue
    })
  }))
